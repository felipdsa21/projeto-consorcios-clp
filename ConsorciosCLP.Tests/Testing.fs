// https://github.com/SuaveIO/suave/blob/v2.7.0-beta1/src/Suave.Tests/Testing.fs
// https://github.com/SuaveIO/suave/blob/v2.7.0-beta1/LICENSE
...
[<AutoOpen>]
module ResponseData =
    let responseHeaders (response: HttpResponseMessage) = response.Headers

    let contentHeaders (response: HttpResponseMessage) = response.Content.Headers

    let statusCode (response: HttpResponseMessage) = response.StatusCode
...
let createClient handler = new HttpClient(handler)
...
let send (client: HttpClient) (timeout: TimeSpan) (ctx: SuaveTestCtx) (request: HttpRequestMessage) =
    ...
    let send =
        client.SendAsync(request, HttpCompletionOption.ResponseContentRead, ctx.cts.Token)
...
let reqResp
    ...
    <| fun ctx ->
        ...
        use request =
            createRequest methd resource query data (endpointUri ctx.suaveConfig)
            |> fRequest
...
let reqCookies methd resource data ctx =
    ...
    reqResp methd resource "" data (Some cookies) DecompressionMethods.None id contentString ctx
...
let reqCookies' methd resource data ctx =
    ...
    |> fun cookies -> cookies.GetCookies(endpointUri ctx.suaveConfig)
